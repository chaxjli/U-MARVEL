...
IND = {
    ...
    "Visual_Grounding": ["MSCOCO" for _ in range(4)] # MSCOCO 数据集有两个版本
}
...
class LazySupervisedDataset(Dataset):
    ...
    def construct_messages(self, data_dict):
        ...
        elif 'txt' in data_dict and 'video' in data_dict:
            message = [
                {
                    "role": "user",
                    "content": [
                        {"type": "video", "video": data_dict['video']},
                        {"type": "text", "text": f"{data_dict['txt']}{self.prompt[0]}"}
                    ]
                },
                {
...
def _load_and_preprocess_image(query_img_path, image_path_prefix):
    ...
    elif isinstance(query_img_path, list):
        full_query_img_path = [os.path.join(image_path_prefix, img_path) for img_path in query_img_path]
        for img_path in full_query_img_path:
            assert os.path.exists(img_path), f"Video Image Path {img_path} does not exist"
    return full_query_img_path

def _prepare_data_dict(txt, img_path, image_path_prefix):
    """Prepare a data dictionary with text and image."""
