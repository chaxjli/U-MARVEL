...
# 定义混合抽样策略
topk_hard_neg_task_name = ['ChartQA', 'CIRR', 'ImageNet_1K', 'InfographicsVQA', 'NIGHTS', 'SUN397', 'VisDial', 'Visual7W', 'VisualNews_i2t', 'VOC2007', 'WebQA']
lastk_hard_neg_task_name =[]
randomk_neg_task_name = ['N24News', 'MSCOCO_t2i', 'A-OKVQA', 'DocVQA', 'HatefulMemes', 'MSCOCO', 'MSCOCO_i2t', 'OK-VQA', 'VisualNews_t2i']
assert len(set(topk_hard_neg_task_name+randomk_neg_task_name)) == 20, "数据集的数量不匹配"
datasetid2name = {
                    0: 'VOC2007', 1: 'N24News', 2: 'SUN397', 3: 'ImageNet_1K', 4: 'HatefulMemes', 5: 'OK-VQA', 
                    6: 'A-OKVQA', 7: 'DocVQA', 8: 'InfographicsVQA', 9: 'ChartQA', 10: 'Visual7W', 11: 'MSCOCO_i2t', 12: 'MSCOCO_t2i', 
                    13: 'VisDial', 14: 'CIRR', 15: 'VisualNews_i2t', 16: 'VisualNews_t2i', 17: 'NIGHTS', 18: 'WebQA', 19: 'MSCOCO'}
name2datasetid = {v: k for k, v in datasetid2name.items()}  # 创建 name2datasetid 字典
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(
        ...
        rerank_scores_path: str,         # rerank scores 地址, 蒸馏 rerank 使用
        ...
        topk_hard_negative = 5,
        topk_modality_hard_negative = 5,
        has_feature_constraint=False,  # 是否使用特征约束, con-train 使用,用来约束当前模型 feature 靠近 epoch1 的 feature
        has_rerank_scores=False,       # 是否使用 rerank scores
    ) -> None:
        ...
        self.cand_cool_dids = list(self.cand_pool.keys())
        ...
        self.has_rerank_scores = has_rerank_scores            # 是否使用 rerank scores
        rank0_print("当前使用的咒语是: ", self.prompt)
        rank0_print("当前数据集是否存在指令: ", self.has_instruction)
        rank0_print("当前候选集 did 的数量: ", len(self.cand_cool_dids))
    ...
    def get_instance(self, index):
        ...
        task_name = datasetid2name[datasetid]
        # 处理 local 负样本数据 ---------------------------------------------------------------------------------------------------------------
        if self.has_hard_negative:
            assert qid == self.hard_negative_data[index]["qid"], f"数据不匹配: {qid} != {self.hard_negative_data[index]['qid']}"
            hard_negative_gt_docs = self.hard_negative_data[index]["gt_docs"]
            hard_negative_top100_docs = self.hard_negative_data[index]["top100_docs"]
            hard_negative_score = self.hard_negative_data[index]["score"]
            if task_name in topk_hard_neg_task_name:
                topk_hard_negative_did_list = _select_topk_hard_neg(hard_negative_gt_docs,hard_negative_top100_docs,hard_negative_score,self.topk_hard_negative)
            elif task_name in lastk_hard_neg_task_name:
                topk_hard_negative_did_list = _select_lastk_hard_neg(hard_negative_gt_docs,hard_negative_top100_docs,hard_negative_score,self.topk_hard_negative)
            else:
                topk_hard_negative_did_list = _select_randomk_neg(qid,hard_negative_gt_docs,self.cand_cool_dids,self.topk_hard_negative)
            if None in topk_hard_negative_did_list:
                topk_hard_negative_did_list = _select_randomk_neg(qid,hard_negative_gt_docs,self.cand_cool_dids,self.topk_hard_negative)
        
        # 处理 global 负样本数据 ---------------------------------------------------------------------------------------------------------------
        if self.has_modality_hard_negative:
            assert qid == self.modality_hard_negative_data[index]["qid"], f"数据不匹配: {qid} != {self.modality_hard_negative_data[index]['qid']}"
            modality_hard_negative_gt_docs = self.modality_hard_negative_data[index]["gt_docs"]
            modality_hard_negative_top100_docs = self.modality_hard_negative_data[index]["top100_docs"]
            modality_hard_negative_score = self.modality_hard_negative_data[index]["score"]
            if task_name in topk_hard_neg_task_name:
                topk_modality_hard_negative_did_list = _select_topk_hard_neg(modality_hard_negative_gt_docs,modality_hard_negative_top100_docs,modality_hard_negative_score,self.topk_modality_hard_negative)
            elif task_name in lastk_hard_neg_task_name:
                topk_modality_hard_negative_did_list = _select_lastk_hard_neg(modality_hard_negative_gt_docs,modality_hard_negative_top100_docs,modality_hard_negative_score,self.topk_modality_hard_negative)
            else:
                topk_modality_hard_negative_did_list = _select_randomk_neg(qid,modality_hard_negative_gt_docs,self.cand_cool_dids,self.topk_modality_hard_negative)
            if None in topk_modality_hard_negative_did_list:
                topk_modality_hard_negative_did_list = _select_randomk_neg(qid,modality_hard_negative_gt_docs,self.cand_cool_dids,self.topk_modality_hard_negative)
    ...
    def __getitem__(self, i):
        ...
        # rank0_print("result_list: ",result_list)
        result_tuple = tuple(result_list)
        # rank0_print("result_tuple: ",result_tuple)
        return result_tuple
    ...
    def select(self, indices):
        ...
        new_dataset.topk_hard_negative = self.topk_hard_negative
        new_dataset.topk_modality_hard_negative = self.topk_modality_hard_negative
        new_dataset.has_feature_constraint = self.has_feature_constraint
        ...
        if self.use_instruction_token and not self.has_instruction:
            raise ValueError("Instruction token is enabled but the dataset does not have instructions.")
        if self.has_hard_negative:
            new_dataset.hard_negative_data = [copy.deepcopy(self.hard_negative_data[i]) for i in indices]
            assert len(new_dataset.hard_negative_data) == len(indices), "hard negative 数据索引错误"
            rank0_print("len(hard_negative_data)",len(new_dataset.hard_negative_data))
        if self.has_modality_hard_negative:
            new_dataset.modality_hard_negative_data = [copy.deepcopy(self.modality_hard_negative_data[i]) for i in indices]
            assert len(new_dataset.modality_hard_negative_data) == len(indices), "modality hard negative 数据索引错误"
            rank0_print("len(modality_hard_negative_data)",len(new_dataset.modality_hard_negative_data))
        if self.has_feature_constraint:
            new_dataset.query_feature = copy.deepcopy(self.query_feature)
            new_dataset.cand_feature = copy.deepcopy(self.cand_feature)
...
def _select_lastk_hard_neg(gt_docs,top100_docs,score,lastk)->List:
    """
    选择 hard negative
    :param gt_docs: 正样本列表
    :param top100_docs: 候选池
    ...
    selected_hard_neg = set()
    first_hard_neg_did = None
    assert lastk > 0, "选取负样本数量必须大于 0"
    assert len(top100_docs) == len(score), "top100_docs 和 score 的长度不一致"
    assert len(top100_docs) > lastk, "top100_docs 的长度必须大于 lastk"
    for i in range(len(top100_docs)-1,-1,-1):
        if top100_docs[i] not in gt_docs:
            selected_hard_neg.add((top100_docs[i]))
            if first_hard_neg_did is None:
                first_hard_neg_did = top100_docs[i]
        if len(selected_hard_neg) >= lastk:
            break
    assert first_hard_neg_did not in gt_docs, "first_hard_neg_did 不能在 gt_docs 中"
    selected_hard_neg = list(selected_hard_neg)
    if len(selected_hard_neg) < lastk:
        selected_hard_neg.extend([first_hard_neg_did] * (lastk - len(selected_hard_neg)))
    return selected_hard_neg

def _select_randomk_hard_neg(gt_docs,top100_docs,score,randomk)->List:
    """
    选择 hard negative
    :param gt_docs: 正样本列表
    :param top100_docs: 候选池
    ...
    """
    selected_hard_neg = set()
    assert randomk > 0, "选取负样本数量必须大于 0"
    assert len(top100_docs) == len(score), "top100_docs 和 score 的长度不一致"
    assert len(top100_docs) > randomk, "top100_docs 的长度必须大于 randomk"
    # 打乱 top100_docs 顺序
    random.shuffle(top100_docs)
    first_hard_neg_did = None
    for i in range(len(top100_docs)):
        if top100_docs[i] not in gt_docs:
            selected_hard_neg.add((top100_docs[i]))
            if first_hard_neg_did is None:
                first_hard_neg_did = top100_docs[i]
        if len(selected_hard_neg) >= randomk:
            break
    assert first_hard_neg_did not in gt_docs, "first_hard_neg_did 不能在 gt_docs 中"
    selected_hard_neg = list(selected_hard_neg)
    if len(selected_hard_neg) < randomk:
        selected_hard_neg.extend([first_hard_neg_did] * (randomk - len(selected_hard_neg)))
    return selected_hard_neg

# 这种抽样方法不允许出现 first_hard_neg_did = None 的情形
def _select_randomk_neg(qid,gt_docs,cand_pool_dids,randomk)->List:
    total_cand = len(cand_pool_dids)
    # 随机生成 100 个索引位置
    random_indices = random.sample(range(total_cand), 100)
    selected_hard_neg = set()
    first_hard_neg_did = None
    assert randomk > 0, "选取负样本数量必须大于 0"
    for i in random_indices:
        if cand_pool_dids[i] not in gt_docs:
            selected_hard_neg.add((cand_pool_dids[i]))
            if first_hard_neg_did is None:
                first_hard_neg_did = cand_pool_dids[i]
        if len(selected_hard_neg) == randomk:
            break
    
    if first_hard_neg_did is None: # 100 如果失败的话，抽取 1000 个 did 进行抽样
        random_indices = random.sample(range(total_cand), 1000)
        for i in random_indices:
            if cand_pool_dids[i] not in gt_docs:
                selected_hard_neg.add((cand_pool_dids[i]))
                if first_hard_neg_did is None:
            ...
            if len(selected_hard_neg) == randomk:
                break
        rank0_print(f"随机抽样失败，重新抽样 1000 个 did。\n"
                    f"当前 qid: {qid}, \n"
                    f"正样本 (gt_docs): {gt_docs}, \n"
                    f"候选池 (cand_pool_dids) 长度: {len(cand_pool_dids)}, \n"
                    f"当前 first_hard_neg_did: {first_hard_neg_did}")
        
    assert first_hard_neg_did not in gt_docs, "first_hard_neg_did 不能在 gt_docs 中"
    assert first_hard_neg_did is not None, "first_hard_neg_did 必须不是 None"
    selected_hard_neg = list(selected_hard_neg)
    if len(selected_hard_neg) < randomk:
        selected_hard_neg.extend([first_hard_neg_did] * (randomk - len(selected_hard_neg)))
    return selected_hard_neg
