...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(
        ...
        image_path_prefix: str = "./data/MMEB-eval/eval_images", 
        rank_num: int = 10,
    ) -> None:
        ...
        with open(ret_query_data_path, 'r') as f: # 读取ret_query_data_path json文件
            self.ret_query_data = json.load(f)

        # self.ret_cand_data = json.load(open(ret_cand_data_path))
        with open(ret_cand_data_path, 'r') as f: # 读取ret_cand_data_path json文件
            self.ret_cand_data = json.load(f)
        rank_num = min(rank_num, len(self.ret_cand_data[0]))  # 确保 rank_num 不超过候选集的长度
        self.rank_num = rank_num 
        self.image_path_prefix = "./data/MMEB-eval/eval_images"
