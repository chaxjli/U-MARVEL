...
class LazySupervisedDataset(Dataset):
    ...
    def get_instance(self, index):
        ...
        if self.has_hard_negative:
            ...
            hard_negative_gt_docs = pos_cand_list[:]
            hard_negative_top100_docs = self.hard_negative_data[index]["did"]
        ...
        if self.has_modality_hard_negative:
            ...
            modality_hard_negative_gt_docs = pos_cand_list[:]
            modality_hard_negative_top100_docs = self.modality_hard_negative_data[index]["did"]
...
def _select_topk_hard_neg(gt_docs,top100_docs,score,topk)->List:
    ...
    assert len(top100_docs) == 50, "top100_docs 的长度必须大于 topk"
...
def _select_lastk_hard_neg(gt_docs,top100_docs,score,lastk)->List:
    ...
    assert len(top100_docs) == len(score), "top100_docs 和 score 的长度不一致"
    assert len(top100_docs) == 50, "top100_docs 的长度必须大于 lastk"
...
def _select_randomk_hard_neg(gt_docs,top100_docs,score,randomk)->List:
    ...
    assert len(top100_docs) == len(score), "top100_docs 和 score 的长度不一致"
    assert len(top100_docs) == 50, "top100_docs 的长度必须大于 randomk"
    # 打乱 top100_docs 顺序
