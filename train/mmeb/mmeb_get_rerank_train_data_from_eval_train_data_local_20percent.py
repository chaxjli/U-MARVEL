...
import argparse
from dataset.datasets_mmeb import QueryDataset, CandidateDataset
...
# ./data/MMEB-train/A-OKVQA/A-OKVQA_query.jsonl
# ./data/MMEB-train/A-OKVQA/A-OKVQA_candidate.jsonl
# ./data/MMEB-train/A-OKVQA/A-OKVQA_qrel.json
# ./data/MMEB-train/mmeb_union_candidate.jsonl
# ./data/MMEB-train/mmeb_union_query.jsonl
query_data_paths = [f"./data/MMEB-train/{subset}/{subset}_query_20percent.jsonl" for subset in IND]
query_cand_pool_paths = ["./data/MMEB-train/mmeb_union_candidate.jsonl"]
cand_pool_paths = [f"./data/MMEB-train/{subset}/{subset}_candidate.jsonl" for subset in IND]
qrels_paths = [f"./data/MMEB-train/{subset}/{subset}_qrel.json" for subset in IND]
    ...
    for query_data_path_id in range(len(query_data_paths)):
        ...
        with torch.no_grad():
            ...
            for batch_idx,batch in enumerate(tqdm(query_dataloader, disable=not is_main_process)):
                ...
                # 修正ID收集：先展平再截取
                # gathered_query_ids = accelerate.utils.gather_object(batch_query_ids)
                # flatten_query_ids = [item for sublist in gathered_query_ids for item in sublist]
                # batch_query_ids = flatten_query_ids[:len(query_embed)]  # 确保与query_embed对齐
        ...
        if query_data_path_id == len(IND)-1:  # 最后一个任务
...
if __name__ == "__main__":
    ...
    parser.add_argument('--image_path_prefix', type=str,default="./data/MMEB-train")
