...
def merge_json_files(args):
    ...
    """
    # path_prefix: ./result/result_eval_train/qwen2-vl-7b_BiLamRA_Ret_cc3m_llm_8m8g_4xlr/local
    ...
    for data_file, cand_name_file, query_name_file in zip(files, cand_names_files, query_names_files):
        try:
            ...
            print(f"data_file: {len(data)}, cand_name_file: {len(data_cand_names)}, query_name_file: {len(data_query_names)}")
            assert  len(data) == len(set(data_query_names)), f"数据不匹配: {len(data)} != {len(set(data_query_names))}"
    ...
    union_up_train_path = "/group/40077/chaxjli/Retrieve/LamRA/data/MMEB-train/mmeb_union_query.jsonl"
    with open("/group/40077/Retrieval_Dataset/MMEB-train/mmeb_union_qid2poslist.json", "r") as f:
        qid2poslist = json.load(f)
    # union_up_train_path = "/group/40077/Retrieval_Dataset/M-BEIR/query/union_train/mbeir_union_up_train_10percent.jsonl"
    ...
    # 处理 rerank_all_data 的 "gt_docs"
    for item in rerank_all_data.keys():
        rerank_all_data[item]["gt_docs"] = qid2poslist[item][:]
    print("rerank_all_data 个数应该是 1011173, 实际是: ", len(rerank_all_data))
    ...
    for item in tqdm(data_query): # data_query 的数据是训练集 query
        ...
        assert len(pos_list) == 1 and pos_list[0] in rerank_all_data[qid]["gt_docs"], f"数据不匹配: {pos_list} != {rerank_all_data[qid]['gt_docs']}"
    ...
    print("data_query_with_top100_score 个数应该是 1068472 ，实际是：", len(data_query_with_top100_score))
    ...
    print("all_data_with_scores 个数应该是 1011173 ，实际是： ", len(all_data_with_scores))
    if "local" in path_prefix:
