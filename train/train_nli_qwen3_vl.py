...
def train():
    ...
    # 统一使用qwen-vl-finetune模型 来训练和评估模型
    ...
    model.use_isotropy_loss = model_args.use_isotropy_loss
    model.use_self_attent_pooling = model_args.use_self_attent_pooling
    model.use_cross_entropy_loss = model_args.use_cross_entropy_loss
    model.use_focal_infonce_loss = model_args.use_focal_infonce_loss
    model.use_diht_loss = model_args.use_diht_loss
    ...
    train_dataset = LazySupervisedDataset(
        ...
        has_instruction = data_args.has_instruction,                       # NLI 默认存在指令,代表是否进行指令匹配的计算
        use_instruction_token = data_args.use_instruction_token,           # NLI 默认不使用指令特殊 token，代表是否使用指令特殊 token
    ...
    rank0_print("data 的长度: ", len(train_dataset.csv))
    # -----------------------------------------------------------
    eval_dataset = None
    ...
    data_collator = COLLATORS[model_args.model_family_id](
        tokenizer=tokenizer,
        processor=processor,
        has_instruction=train_dataset.has_instruction,                       # NLI 数据集是否做指令匹配
        use_instruction_token=train_dataset.use_instruction_token,           # NLI数据集是否使用指令特殊 token
        has_hard_negative=train_dataset.has_hard_negative,                   # NLI数据集使用 hard negative
        has_modality_hard_negative=train_dataset.has_modality_hard_negative, # NLI数据集使用 modality hard negative
    )
...
if __name__ == "__main__":
    rank0_print("当前脚本的运行时间是:", time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time())))
    train()