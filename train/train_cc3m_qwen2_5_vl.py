...
from dataset.datasets_cc3m import LazySupervisedDataset
from loaders import LOADERS
from supported_models import MODULE_KEYWORDS
from utils import (
    rank0_print, find_all_linear_names, safe_save_model_for_hf_trainer,
    get_peft_state_maybe_zero_3, TrainerWithCustomSampler,rank0_print_model_memory_summary,rank0_print_module_memory,
)
...
def train():
    ...
    # 修改模型的配置
    model, tokenizer, processor = loader.load(pretrain=False,use_bi_atten=model_args.use_bi_atten)
    tokenizer.model_max_length = training_args.model_max_length
    model.mean_pooling = model_args.mean_pooling
    model.use_bi_atten = model_args.use_bi_atten
    model.use_latent_atten = model_args.use_latent_atten
    model.use_instruction_mask = model_args.use_instruction_mask
    model.use_bi_loss = model_args.use_bi_loss
    ...
    rank0_print("use_latent_atten: ",model.use_latent_atten,"use_instruction_mask: ",model.use_instruction_mask)
    ...
    # # ---------------------------------------------------------------------------------------------------
    # rank0_print("*"*100)
    # rank0_print("打印模型的显存信息：")
    # rank0_print_model_memory_summary(model)
    # rank0_print("*"*100)
    # # ---------------------------------------------------------------------------------------------------
    
    # 这里对 sim.temp 进行重新赋值，确保温度参数初始化正常
    with torch.no_grad():
    ...
    # use_lora, train_vision_encoder, use_vision_lora 都是 False, 才会执行 rank0_print("No LoRA enabled...")
    if not (lora_args.use_lora or (training_args.train_vision_encoder and lora_args.use_vision_lora)):
        ...
        if training_args.train_vision_encoder and lora_args.use_vision_lora:
            ...
            rank0_print(find_all_linear_names(named_modules, vision_encoder_keys))
            rank0_print("*"*100)
            ...
            rank0_print("vision_encoder_keys",vision_encoder_keys)
            rank0_print("find_all_linear_names(named_modules, vision_encoder_keys): ",find_all_linear_names(named_modules, vision_encoder_keys))
            rank0_print("不允许全量微调 Vision Encoder，请将 USE_VISION_LORA 设置为 True")
            rank0_print("*"*100)
            
            # 暂时不允许全量微调 Vision Encoder
            # full_modules.extend(vision_encoder_keys) # 这种方法全量微调 vision_encoder 有问题，所以将 USE_VISION_LORA 设置为 False 会报错
            # full_modules.extend(find_all_linear_names(named_modules, vision_encoder_keys))
            
        
        if lora_args.use_lora:
            rank0_print("LoRA for LLM enabled...")
            rank0_print(find_all_linear_names(named_modules, llm_keys))
            rank0_print("*"*100)
            lora_modules.extend(find_all_linear_names(named_modules, llm_keys))
        else:
            rank0_print("LLM will be fully trained...")
            rank0_print("llm_keys",llm_keys)
            rank0_print("find_all_linear_names(named_modules, llm_keys): ",find_all_linear_names(named_modules, llm_keys))
            rank0_print("不允许全量微调 LLM，显存会超出限制")
            rank0_print("*"*100)
            # 暂时不允许全量微调 LLM
            # full_modules.extend(llm_keys) # 这种方法全量微调 llm 有问题，所以将 use_lora 设置为 False 会报错
            # full_modules.extend(find_all_linear_names(named_modules, llm_keys))
        
        if training_args.train_vision_projector:
            rank0_print("Vision projector will be fully trained...")
            rank0_print("vision_projector_keys: ",vision_projector_keys)
            rank0_print("find_all_linear_names(named_modules, llm_keys): ",find_all_linear_names(named_modules, llm_keys))
            rank0_print("*"*100)
    ...
    rank0_print("*"*100)
        ...
        # # 打印添加 LoRA 后模型的显存信息-------------------------------------
        # rank0_print("*"*100)
        # rank0_print("打印添加 LoRA 后模型的显存信息: ")
        # rank0_print_model_memory_summary(model)
        # rank0_print("*"*100)
        # # -----------------------------------------------------------
    ...
    data_collator = COLLATORS[model_args.model_family_id](
        tokenizer=tokenizer,
        processor=processor,
        has_instruction=train_dataset.has_instruction, 
        use_instruction_token=train_dataset.use_instruction_token, 
        has_hard_negative=train_dataset.has_hard_negative, 
        has_modality_hard_negative=train_dataset.has_modality_hard_negative, 
    )
    ...
    # # 截取样本，用于测试，实际训练时请删除-----------------------------------------------------
    # random.seed(42)
    # indices = random.sample(range(len(train_dataset.all_data)), 60)
    # train_dataset_subset = train_dataset.select(indices)
    # rank0_print("train_dataset 的长度：", len(train_dataset))
    # rank0_print("train_dataset_subset 的长度：", len(train_dataset_subset))
    # # -----------------------------------------------------------------------------------
    ...
    if dist.is_initialized():
        if dist.get_rank() == 0 and model.use_latent_atten:
            ...
            torch.save(self.latent_attention.state_dict(), latent_path)
