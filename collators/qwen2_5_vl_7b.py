...
from .qwen2_5_vision_process import process_vision_info,set_max_pixels
# from qwen_vl_utils import process_vision_info
# from .qwen2_5_vision_process import set_max_pixels


@register_collator("qwen2_5-vl-7b")
class Qwen2_5_VL7BDataCollator(BaseDataCollator):
    def __init__(self,tokenizer, processor,has_instruction,use_instruction_token, \
        ...
        self.has_distill_with_pos = has_distill_with_pos                # 是否使用 infonce loss 进行辅助蒸馏
        ...
        if self.is_reset_max_pixels and max_pixel is not None:
            rank0_print("Warning: is_reset_max_pixels 为 True 时，max_pixel 参数已传入，使用传入的 max_pixel 参数设置最大像素值{}*{}".format(max_pixel, max_pixel))
            set_max_pixels(max_pixel*max_pixel)
        elif self.is_reset_max_pixels and max_pixel is None: 
            rank0_print("Warning: is_reset_max_pixels 为 True，且未传入 max_pixel 参数，使用默认的 256*256 作为最大像素值")
            # set_max_pixels(256*256)      # qwen2_5 vl 7b 使用更大的图片尺寸训练
    ...
    def __call__(self, messages: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        if self.has_distill_with_pos:
            new_messages_category_size -=1    # 如果使用正样本进行辅助蒸馏，需要传入正样本矩阵 pos_index_list
        ...
        for idx_fs in range(new_messages_category_size, category_size):
            for item in messages:
                ...
                if "pos_index" in item[idx_fs]:
                    pos_index_list.append(item[idx_fs]["pos_index"])
        ...
        if self.has_feature_constraint:
            ...
            if self.has_rerank_scores:
                if self.has_distill_with_pos:
                    feature_list = feature_list[:batch_size] + feature_list[2*batch_size:] + feature_list[batch_size:2*batch_size]
                else:
        ...
        if self.has_rerank_scores:
            if self.has_distill_with_pos:
                # 保留正样本后，应该是 topk_hard_neg, topk_modality_hard_neg, pos,这是为了适配后面的代码
                # 模型训练的时候会处理成匹配 scores列表 [pos_score, topk_hard_neg_score1, topk_hard_neg_score2, ..., topk_modality_hard_neg_score1, ...]
                assert len(scores_list[0]) == new_messages_category_size - 1 
                assert len(scores_list) == len(messages)
                assert len(new_messages) == new_messages_category_size * len(messages)
                new_messages = new_messages[:batch_size] + new_messages[2*batch_size:] + new_messages[batch_size:2*batch_size]
            else:
                # 去除正样本后，应该是 topk_hard_neg, topk_modality_hard_neg
        ...
        if self.has_instruction and not self.use_instruction_token:
            ...
            for idx, item in enumerate(messages):
                ...
                instruction_tokens = self.tokenizer(instruction, truncation=True, max_length=10, padding=False, return_tensors="pt", add_special_tokens=False)
        ...
        return dict(
            ...
            pos_index_list=pos_index_list if self.has_distill_with_pos else None,
        )
