...
from collators.eval_rerank_qwen3_vl import EvalRerankDataCollator
...
def rerank(args):
    ...
    path_name_fix = args.path_name_fix 
    ...
    if args.save_name.startswith('urban1k_t2i'):
        ...
        dataset = MSRVTTRerankT2VDataset(data_path,video_data_path,ret_query_data_path, ret_cand_data_path, rank_num=rank_num)
    elif args.save_name.startswith('msvd'):
        dataset = MSVDRerankT2VDataset(ret_query_data_path, ret_cand_data_path, video_path_prefix,test_video_path,captions_path,rank_num=rank_num)
        ...
        dataset = CIRCORerankDataset(ret_query_data_path, ret_cand_data_path, annotation_path, image_path_prefix, split='val', rank_num=rank_num)
    ...
    #     dataset = VistRerankDataset(args.data_path, args.image_path_prefix, args.ret_query_data_path, args.ret_cand_data_path, args.rank_num)
        ...
        raise ValueError(f"Unknown dataset name: {args.save_name}")
    
    if args.save_name.startswith('msrvtt'):
        data_collator = EvalRerankDataCollator(tokenizer=tokenizer, processor=processor, is_reset_max_pixels=False)
    else:
    ...
    if is_main_process:
        ...
        if not args.save_name.startswith('sugar_crepe'):
            with open(f"./{args.path_name_fix}/{args.save_name}_test_queryid2rerank_score.json", 'w') as f:
                json.dump(queryid2rerank_score, f, indent=2)
        else:
            with open(f"./{args.path_name_fix}/{args.save_name}_{args.data_type}_test_queryid2rerank_score.json", 'w') as f:
                json.dump(queryid2rerank_score, f, indent=2)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--video_path_prefix', type=str, default=None)
    parser.add_argument('--test_video_path', type=str, default=None)
    parser.add_argument('--captions_path', type=str, default=None)
    parser.add_argument('--video_data_path', type=str, default=None)
    parser.add_argument('--task_name', type=str)
    parser.add_argument('--path_name_fix', type=str)
