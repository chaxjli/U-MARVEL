...
subset_names = subset_names[::-1]  # 反转 subset_names 列表，确保 OOD 在前面
# ------------------------------------------------------------------------------------------------
...
def eval(args):
    ...
    for idx, subset in enumerate(subset_names):
        ...
        encode_output_path = os.path.join(args.save_dir_name, "mmeb_cand_without_inst")
        ...
        cand_dataset = MMEBEvalDataset(
            ...
            has_instruction=False,  # 这里设置为 False，因为候选集不需要指令
            use_instruction_token=False,  # 这里设置为 False，因为候选集不需要指令
        )
