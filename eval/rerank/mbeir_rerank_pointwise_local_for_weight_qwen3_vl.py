...
# rerank_model_name = "qwen2-vl-7b_BiLamRA_Ret_cc3m_llm_8m8g_4xlr_train_temperature_global_mixed_hard_neg_all_8m8g_1point5e-5-two_strategy_train_temperature_continue_distillrerank-8m8g_1e-5-Rank-Only-Pointwise"
model_name = "qwen3-vl-4b_m-beir_stage2_model"                              # 这里需要修改: 需要第一阶段模型名称
rerank_model_name = "qwen3-vl-4b_m-beir_stage1_model-Rank-Only-Pointwise"   # 这里需要修改: 需要第二阶段 rerank 模型名称
task_names = [
...
for weight_param in weight_params:
    ...
    for task_name in task_names:
        ...
        raw_scores = load_json(f"./result/{model_name}_eval_results_finetune/local/mbeir_{task_name}_test_{model_name}_scores.json")
        rerank_scores = load_json(f"./result/result_rank/{rerank_model_name}/{model_name}/local/{task_name}_top50_test_queryid2rerank_score.json")
        query_names = load_json(f"./result/{model_name}_eval_results_finetune/local/mbeir_{task_name}_test_{model_name}_query_names.json")
        cand_names = load_json(f"./result/{model_name}_eval_results_finetune/local/mbeir_{task_name}_test_{model_name}_cand_names.json")
...
# 保存最佳结果到文件
# 删除旧的pointwise_recall_results.txt文件
pointwise_results_file = os.path.join(rerank_path_prefix,'pointwise_recall_results.txt')
if os.path.exists(pointwise_results_file):
    os.remove(pointwise_results_file)
