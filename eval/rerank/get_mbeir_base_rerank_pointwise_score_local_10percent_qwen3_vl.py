...
import copy
# 这里需要修改: 需要第一阶段模型名称
# model_name = "qwen2-vl-7b_BiLamRA_Ret_cc3m_llm_8m8g_4xlr_train_temperature_global_mixed_hard_neg_all_8m8g_1point5e-5-two_strategy_train_temperature"   
# rerank_model_name = "qwen3-vl-4b_m-beir_stage1_model-Rank-Only-Pointwise"

model_name = "qwen3-vl-4b_m-beir_stage2_model"
...
rerank_path_prefix = f"./result/score/{rerank_model_name}/{model_name}/local/10percent"
...
for task_name in task_names:
    ...
    raw_scores = load_json(f"./result/result_eval_train/{model_name}/local/10percent/mbeir_train_{task_name}_{model_name}_scores.json")
    rerank_scores = load_json(f"./result/result_rank/train/{rerank_model_name}/{model_name}/local/10percent/{task_name}_top50_train_queryid2rerank_score.json")
    query_names = load_json(f"./result/result_eval_train/{model_name}/local/10percent/mbeir_train_{task_name}_{model_name}_query_names.json")
    cand_names = load_json(f"./result/result_eval_train/{model_name}/local/10percent/mbeir_train_{task_name}_{model_name}_cand_names.json")
