...
retrieval_path_name_fix=f"./result/{source_model_name}_eval_results_finetune/zeroshot/multiturn_fashion"
...
for ind, query_name in enumerate(tqdm(query_names)):
    relevant_docs = [mrf_data[ind]['target'][1]]
...
with open(save_path_name_fix, 'w') as f:
    f.write(f"multi-turn fashion retrieval all evaluation\n")
    for k in k_lists:
