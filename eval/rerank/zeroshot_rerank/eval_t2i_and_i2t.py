...
retrieval_path_name_fix=f"./result/{source_model_name}_eval_results_finetune/zeroshot/{task_name[:-4]}"
...
with open(save_path_name_fix, 'w') as f:
    f.write(f"{task_name[:-4]} evaluation\n")
    for k in k_lists:
        if "i2t" in task_name:
            f.write(f"text_retrieval_recall@{k} = {sum(res[f'recall_{k}']) / len(res[f'recall_{k}'])}\n")
        else:
            f.write(f"image_retrieval_recall@{k} = {sum(res[f'recall_{k}']) / len(res[f'recall_{k}'])}\n")