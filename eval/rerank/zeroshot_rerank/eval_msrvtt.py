...
retrieval_path_name_fix=f"./result/{source_model_name}_eval_results_finetune/zeroshot/msrvtt"
rerank_path_name_fix=f"./result/result_rank/{model_name}/{source_model_name}/zeroshot/msrvtt_t2v"
...
# 权重从 0到10 每个 0.1 遍历
# weights = [0.1 * i for i in range(21)]
weights = [5]
for weight in weights:
    ...
    for idx, query_name in enumerate(query_names):
        ...
        final_score = [weight * raw_score[index] + 1.0 * rerank_score[index] for index in range(len(raw_score))]
    ...
    text2video_gt_index = [i for i in range(len(rerank_candidate_names))]
    k_lists = [1,5,10]
    # k_lists = [1]
    res = {}
