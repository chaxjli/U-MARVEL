...
rerank_model_name = "qwen2-vl-7b_BiLamRA_Ret_cc3m_llm_8m8g_4xlr-Rank-Only-Pointwise_global"           # 这里需要修改: 需要第二阶段 rerank 模型名称
...
for task_name in task_names:
    ...
    with open(f"{rerank_path_prefix}/{task_name}_fusion_scores2.json", "w") as f:
        json.dump(fusion_scores, f,ensure_ascii=False)
    with open(f"{rerank_path_prefix}/{task_name}_fusion_query_names2.json", "w") as f:
        json.dump(fusion_query_names, f, ensure_ascii=False)    
    with open(f"{rerank_path_prefix}/{task_name}_fusion_cand_names2.json", "w") as f:
        json.dump(fusion_cand_names, f, ensure_ascii=False)
    print(f"融合后的结果已保存到 {rerank_path_prefix}/{task_name}_fusion_scores2.json")
    print(f"融合后的结果已保存到 {rerank_path_prefix}/{task_name}_fusion_query_names2.json")
    print(f"融合后的结果已保存到 {rerank_path_prefix}/{task_name}_fusion_cand_names2.json")
...
with open(f"{rerank_path_prefix}/fusion_results2.jsonl", "w") as f:
    for item in fusion_results_list:
        f.write(json.dumps(item, ensure_ascii=False) + "\n")
print(f"融合后的结果已保存到 {rerank_path_prefix}/fusion_results2.jsonl")
