...
image_path_prefix = "./data/M-BEIR/"  # debug
...
def eval(args):
    ...
    query_dataset = QueryDataset(
        ...
        image_path_prefix=image_path_prefix                   #  debug
    )
    ...
    with torch.no_grad():
        ...
        for batch in tqdm(query_dataloader, disable=not is_main_process):
            ...
            # 在特征处理循环中立即转存 CPU debug
            query_features.append(query_embed.cpu())  # 替换原append语句

    # NPU 显存不够移动到 CPU 上面进行合并最后一个任务  "./data/M-BEIR/qrels/test/mbeir_edis_task2_test_qrels.txt"
    # 新增代码：安全释放模型内存
    print("释放前NPU内存:", torch.npu.memory_allocated()/1e9, "GB")
    # 新增代码：彻底释放模型 ----------------------
    model = model.to("cpu")  # 先将模型移出NPU
    del model
    accelerator.free_memory()  # 关键！释放加速器持有的资源
    torch.npu.empty_cache()
    print("释放后NPU内存:", torch.npu.memory_allocated()/1e9, "GB")    
    ...
    if is_main_process:
        ...
        save_dir_name = "./LamRA_Ret_eval_results_author_model"
