...
def eval(args):
    ...
    if "qwen3-vl" in model_id:
        model = Qwen3VLRetForConditionalGeneration.from_pretrained(
            ...
            use_bi_atten=args.use_bi_atten=='True', 
        ...
        model = Qwen2_5_VLRetForConditionalGeneration.from_pretrained(
         ...
         raise NotImplementedError("Only qwen2-vl, qwen3-vl, qwen2_5-vl models are supported currently.") 
    # 重置模型的参数
    ...
    with torch.no_grad():
        ...
        for batch in tqdm(query_dataloader, disable=not is_main_process):
            ...
            # rank0_print("#"*100)
            # rank0_print("打印模型返回的结果：",model(**batch, inference=True))
