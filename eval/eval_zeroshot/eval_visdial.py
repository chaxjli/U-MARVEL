...
import torch 
import argparse
from dataset.datasets_visdial import VisDialDataset
...
def eval(args):
    ...
    if hasattr(model, '_set_model_use_bi_atten'):
    ...
    # ------------------------------------------------------------------------------------------------
    query_dataset = VisDialDataset(
        image_path_prefix=args.image_path_prefix, 
        data_path=args.data_path,
        type='image',
    )

    cand_dataset = VisDialDataset(
        image_path_prefix=args.image_path_prefix, 
        data_path=args.data_path,
        type='text',
    )
    ...
    if is_main_process:
        ...
        import numpy as np 
        print(f'len query_ids: {len(query_ids)}')
        print(f'len candidate_ids: {len(candidate_ids)}')
        ...
        with open(f"{save_dir_name}/{model_name}.txt", 'w') as f:
            f.write('visdial evaluation' + '\n')
            for key in metrics:
                f.write(f"{key} = {metrics[key]}" + "\n")

        if args.save_for_rerank:
            save_for_rerank(query_ids, candidate_ids, scores, save_dir_name)

def save_for_rerank(query_ids, candidate_ids, initial_scores, save_dir_name):
    index = []
    scores = []
    ...
    with open(f"{save_dir_name}/visdial_query_names.json", 'w') as f:
        json.dump(query_ids.tolist(), f, indent=2)
    with open(f"{save_dir_name}/visdial_cand_names.json", 'w') as f:
        json.dump(cand_names.tolist(), f, indent=2)
    with open(f"{save_dir_name}/visdial_scores.json", 'w') as f:
