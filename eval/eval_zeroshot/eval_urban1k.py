...
from models.qwen2_vl import Qwen2VLRetForConditionalGeneration                  # 注意使用这个模型一个input允许有多个image
from models.qwen2_vl_finetune import Qwen2VLRetFinetuneForConditionalGeneration # 注意使用这个模型一个input只能有一个image
...
import argparse
from dataset.datasets_urban1k import UrbanDataset
from collators.eval_collator_default_resolution import EvalDataCollator
...
def eval(args):
    ...
    if "qwen3-vl" in model_id:
        ...
        raise NotImplementedError("Only qwen2-vl and qwen3-vl models are supported currently.")
    ...
    if "qwen2-vl" in model_id:
        ...
        rank0_print("model.use_bi_atten: ", model.use_bi_atten)
    rank0_print("-"*50)
    ...
    if is_main_process:
        ...
        for recall_k in recall_k_list:
            ...
            # Also, the number of positives are the total number of texts matching the image in the dataset, 
            # as we have a set of captions
            # for each image, that number will be greater than 1 for text retrieval.
            # However, image/text retrieval recall@k, the way it is done in CLIP-like papers, is a bit different.
            # recall@k, in CLIP-like papers, is, for each image, either 1 or 0. 
            # It is 1 if atleast one text matches the image among the top-k.
...
def save_for_rerank_t2i(query_ids, candidate_ids, initial_scores, save_dir_name):
    ...
    with open(f"{save_dir_name}/urban1k_t2i_query_names.json", 'w') as f:
        json.dump(query_names.tolist(), f, indent=2)
    with open(f"{save_dir_name}/urban1k_t2i_cand_names.json", 'w') as f:
        json.dump(cand_names.tolist(), f, indent=2)
    with open(f"{save_dir_name}/urban1k_t2i_scores.json", 'w') as f:
...
def save_for_rerank_i2t(query_ids, candidate_ids, initial_scores, save_dir_name):
    ...
    cand_names = np.array([[candidate_ids[item] for item in row] for row in index])
    query_names = query_ids 
    with open(f"{save_dir_name}/urban1k_i2t_query_names.json", 'w') as f:
        json.dump(query_names.tolist(), f, indent=2)
    with open(f"{save_dir_name}/urban1k_i2t_cand_names.json", 'w') as f:
        json.dump(cand_names.tolist(), f, indent=2)
    with open(f"{save_dir_name}/urban1k_i2t_scores.json", 'w') as f:
        json.dump(scores, f, indent=2)
