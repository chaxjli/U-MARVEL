...
class Qwen2_5_VLRetFinetuneForConditionalGeneration(BiQwen2_5_VLForConditionalGeneration):
    def __init__(self, config,use_bi_atten=True,temp=0.05):
        ...
        rank0_print("Qwen2_5_VLRetFinetuneForConditionalGeneration 模型初始化 self.use_bi_atten:", self.use_bi_atten)
    ...
    def forward(
        ...
        if self.mean_pooling:
            ...
            # rank0_print(f"self.mean_pooling is {self.mean_pooling}, 使用 embed token 提取特征")
            # rank0_print("self.use_bi_atten is ", self.use_bi_atten)
        ...
        # # -------------------------------------------------------
        if inference:
            # rank0_print("qwen2_5_vl_finetune 的 ids 的长度：", len(ids))
            if ids is not None:
