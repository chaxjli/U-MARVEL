...
class Qwen3VLRetFinetuneHardNegForConditionalGeneration(BiQwen3VLForConditionalGeneration):
    ...
    def forward(
            ...
            if inputs_embeds is None:
                ...
                if pixel_values is not None:
                    ...
                    if current_image_num != 0:
                        ...
                        batch_image_embeds,deepstack_image_embeds = self.visual(batch_pixel_values, grid_thw=image_grid_thw[image_nums:image_nums + current_image_num])
        ...
        if self.ignore_batch_other_samples:
            cos_sim = self.sim(embed1, embed2,sim_compute="hadamard_product") # 计算 query 和正样本 pos 的相似度
            if self.use_bi_loss: # 如果使用双向损失
                inverse_cos_sim = self.sim(embed2, embed1,sim_compute="hadamard_product")
            if has_hard_negative:
                for embed3 in embed_topk_hard_negative:
                    ...
                    if self.use_bi_loss:
                        embed2_embed3_cos = self.sim(embed2, embed3,sim_compute="hadamard_product")
            ...
            if has_modality_hard_negative:
                for embed4 in embed_topk_modality_hard_negative:
                    ...
                    if self.use_bi_loss:
                        embed2_embed4_cos = self.sim(embed2, embed4,sim_compute="hadamard_product")
            ...
            if self.use_bi_loss:
                if (inverse_cos_sim < -1.1/temp).any() or (inverse_cos_sim > 1.1/temp).any():
        ...
        # 如果没有忽略 batch 其他负样本，计算余弦相似度   *************************************************************
            ...
            cos_sim = self.sim(embed1, embed2)  # (B, B) 使用自定义的相似度计算模块
            if self.use_bi_loss:                # 如果使用双向损失
