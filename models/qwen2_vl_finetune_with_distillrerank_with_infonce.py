...
class Qwen2VLRetFinetuneDistillRerankForConditionalGeneration(BiQwen2VLForConditionalGeneration):
    def __init__(self, config):
        ...
        self.distill_with_infonce = False        # 是否使用 infonce loss 进行辅助蒸馏
    ...
    def forward(
        ...
        if dist.is_initialized():
            ...
            if self.distill_with_infonce:
                ...
                all_embed1 = torch.cat(embed1_list, 0)

            # 如果使用 hard negative , 处理 embed3 ------------------------------------------------------------------
            # embed1 的维度是 (batch_size, embed_dim)；embed3 的维度是 (batch_size, embed_dim)
            # 但是 embed3 有 topk_hard_negative 个
            if has_hard_negative:
                ...
                if self.distill_with_infonce:
                    embed_topk_hard_negative_temp = torch.cat(embed_topk_hard_negative,0)
                    embed_topk_hard_negative_list = [torch.zeros_like(embed_topk_hard_negative_temp) for _ in range(dist.get_world_size())]
                    dist.all_gather(tensor_list=embed_topk_hard_negative_list, tensor=embed_topk_hard_negative_temp.contiguous())
                    embed_topk_hard_negative_list[dist.get_rank()] = embed_topk_hard_negative_temp
                    all_embed_topk_hard_negative = torch.cat(embed_topk_hard_negative_list, 0)
            ...
            if has_modality_hard_negative:
                ...
                if self.distill_with_infonce:
                    # 跨 GPU 收集模态负样本
                    embed_topk_modality_hard_negative_temp = torch.cat(embed_topk_modality_hard_negative, 0)
                    embed_topk_modality_hard_negative_list = [torch.zeros_like(embed_topk_modality_hard_negative_temp) for _ in range(dist.get_world_size())]
                    dist.all_gather(tensor_list=embed_topk_modality_hard_negative_list, tensor=embed_topk_modality_hard_negative_temp.contiguous())
                    embed_topk_modality_hard_negative_list[dist.get_rank()] = embed_topk_modality_hard_negative_temp
                    all_embed_topk_modality_hard_negative = torch.cat(embed_topk_modality_hard_negative_list, 0)

            if self.distill_with_infonce:
                # 收集正样本
                assert len(embed_features) % batch_size == 0, "embed_features 的长度必须是 batch_size 的整数倍"
                bs_num = len(embed_features) // batch_size
                embed1_pos = embed_features[(bs_num-1)*batch_size:bs_num*batch_size]
                # 跨 GPU 收集正样本
                embed1_pos_list = [torch.zeros_like(embed1_pos) for _ in range(dist.get_world_size())]
                dist.all_gather(tensor_list=embed1_pos_list, tensor=embed1_pos.contiguous())
                embed1_pos_list[dist.get_rank()] = embed1_pos
                all_embed1_pos = torch.cat(embed1_pos_list, 0)
        ...
        if has_hard_negative:
            for embed3 in embed_topk_hard_negative:
                ...
                if cos_sim is None:
                    cos_sim = compute_cos_sim_ignore_batch_other_samples(embed1, embed3)
                else:
        ...
        if has_modality_hard_negative:
            for embed4 in embed_topk_modality_hard_negative:
                ...
                if cos_sim is None:
                    cos_sim = compute_cos_sim_ignore_batch_other_samples(embed1, embed4)
                else:
        ...
        if (rerank_scores < -1).any() or (rerank_scores > 1).any():
