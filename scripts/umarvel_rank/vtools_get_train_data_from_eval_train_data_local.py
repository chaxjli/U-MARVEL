...
# 模型评估参数   -----  请根据实际情况修改
MODEL_NAME="qwen2-vl-7b_umarvel_progressive_transition_m-beir"                           # 模型名称
MODEL_ID = f"./checkpoints/{MODEL_NAME}"
ORIGINAL_MODEL_ID ="./checkpoints/qwen2-vl-7b_umarvel_progressive_transition_cc3m-merged"     # 原始模型路径
...
# 创建结果目录
os.makedirs(f"./result/result_eval_train/{MODEL_NAME}/local", exist_ok=True)
total_log_file = f"./result/result_eval_train/{MODEL_NAME}/local/{TIMENAME}total_stdout.log"
...
command = [
    ...
    "train/mbeir_get_rerank_train_data_from_eval_train_data_local.py",
    "--instructions_path=./data/M-BEIR/instructions/query_instructions.tsv",
    f"--save_dir_name=./result/result_eval_train/{MODEL_NAME}/local",
