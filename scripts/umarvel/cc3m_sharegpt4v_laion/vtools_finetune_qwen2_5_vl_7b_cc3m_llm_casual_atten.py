...
MODEL_LOCAL_PATH = "./checkpoints/qwen2_5-vl-7b_nli_pertrain_stage1_model_casual_atten_lr4e-4_bsz72_epoch2_lora64_alpha128-merged"
RUN_ID = f"{MODEL_ID}_cc3m_llm_stage1_model_casual_atten_lr2e-4_bsz60_epoch1_lorar32_alpha64_2m8g"
DATA_PATH = "./data/cc3m-595k/train_anno.json"     # 训练数据集路径
...
# 定义训练参数
# 开启 TRAIN_VISION_ENCODER 会训练视觉编码器，不开启会使用预训练的视觉编码器
# 开启 USE_VISION_LORA 会使用视觉 LoRA，不开启会全参数微调

# 原来的参数数 lora_r=32 lora_alpha=64 lr=1e-4 bsz=60 epoch=1
TRAIN_VISION_ENCODER = False        # 是否训练视觉编码器，开启
USE_VISION_LORA = False             # 是否使用视觉 LoRA，开启
TRAIN_VISION_PROJECTOR = False      # 是否训练视觉投影器，开启
USE_LORA = True                     # 是否使用 LoRA，开启
Q_LORA = False
LORA_R = 32
LORA_ALPHA = 64
...
print("QUERY_DATASET_HAS_INSTRUCTION: ",QUERY_DATASET_HAS_INSTRUCTION)
print("QUERY_DATASET_USE_INSTRUCTION_TOKEN: ",QUERY_DATASET_USE_INSTRUCTION_TOKEN)
print("HAS_HARD_NEGATIVE: ",HAS_HARD_NEGATIVE)
print("HAS_MODALITY_HARD_NEGATIVE: ",HAS_MODALITY_HARD_NEGATIVE)
print("MODEL_MEAN_POOLING: ",MODEL_MEAN_POOLING)
print("MODEL_USE_BI_ATTEN: ",MODEL_USE_BI_ATTEN)
print("MODEL_USE_LATENT_ATTEN: ",MODEL_USE_LATENT_ATTEN)
print("MODEL_USE_INSTRUCTION_MASK: ",MODEL_USE_INSTRUCTION_MASK)
print("TRAIN_VISION_ENCODER: ",TRAIN_VISION_ENCODER)
print("USE_VISION_LORA: ",USE_VISION_LORA)
print("TRAIN_VISION_PROJECTOR: ",TRAIN_VISION_PROJECTOR)
...
torchrun_command = [
    ...
    *DISTRIBUTED_ARGS.split(),
    "train/train_cc3m_qwen2_5_vl.py",
    "--model_id", MODEL_ID,
    "--data_path", DATA_PATH,
