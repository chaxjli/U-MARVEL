...
RUN_ID = f"{MODEL_ID}_BiLamRA_Ret_cc3m_llm_8m8g_4xlr_train_temperature_global_mixed_hard_neg_all_8m8g_1point5e-5-two_strategy_train_temperature_held-out" # 模型名称
SOURCE_MODEL_NAME = "qwen2-vl-7b_BiLamRA_Ret_cc3m_llm_8m8g_4xlr_train_temperature_held-out"                                           # 负样本的来源
MODEL_LOCAL_PATH = "./checkpoints/held-out/qwen2-vl-7b_BiLamRA_Ret_cc3m_llm_8m8g_4xlr_train_temperature_held-out-merged"              # 基座模型
QUERY_DATA_PATH = "./data/M-BEIR/query/union_train/mbeir_union_up_train_held-out.jsonl"
...
MODEL_USE_RERANK_SCORES = False                    # 是否使用重排序分数

# 负样本相关参数
HAS_HARD_NEGATIVE = False                           # 是否使用 hard negative
HAS_MODALITY_HARD_NEGATIVE = True                   # 是否使用 modality hard negative
MODEL_TOPK_HARD_NEGATIVE = 5
MODEL_TOPK_MODALITY_HARD_NEGATIVE = 5
HARD_NEGATIVE_PATH = f"./result/result_eval_train/held-out/{SOURCE_MODEL_NAME}/local/rerank_data_all_eval_train_local.jsonl"
MODALITY_HARD_NEGATIVE_PATH = f"./result/result_eval_train/held-out/{SOURCE_MODEL_NAME}/global/rerank_data_all_eval_train_global.jsonl"
QUERY_FEATURE_PATH = f"./result/result_eval_train/held-out/{SOURCE_MODEL_NAME}/global/all_query_names2query_features.pth" #  query 特征约束路径
CAND_FEATURE_PATH = f"./result/result_eval_train/held-out/{SOURCE_MODEL_NAME}/global/all_cand_ids2cand_features.pth"      #  cand 特征约束路径
MODEL_IGNORE_BATCH_OTHER_SAMPLES = False            # 是否忽略批次中的其他样本
...
PER_DEVICE_BATCH_SIZE = 17   # 实验的时候记得修改成 17，保持 120 的特征数量
GRAD_ACCUM = 1
NUM_EPOCHS = 1
LR = 1.5e-5
MODEL_MAX_LEN = 1024
...
torchrun_command = [
    ...
    *DISTRIBUTED_ARGS.split(),
    "train/train_mbeir_with_mixed_hard_neg_sample.py",
    "--model_id", MODEL_ID,
...
try:
    # 运行命令
    print(f"开始执行命令: {redirect_command}")
