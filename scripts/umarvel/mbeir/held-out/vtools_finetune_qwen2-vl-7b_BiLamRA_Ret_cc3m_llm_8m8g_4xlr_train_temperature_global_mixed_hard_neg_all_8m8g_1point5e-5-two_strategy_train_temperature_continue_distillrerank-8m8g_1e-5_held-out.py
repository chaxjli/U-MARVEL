...
RUN_ID = f"{MODEL_ID}_BiLamRA_Ret_cc3m_llm_8m8g_4xlr_train_temperature_global_mixed_hard_neg_all_8m8g_1point5e-5-two_strategy_train_temperature_continue_distillrerank-8m8g_1e-5_held-out"     # 模型名称
SOURCE_MODEL_NAME = "qwen2-vl-7b_BiLamRA_Ret_cc3m_llm_8m8g_4xlr_train_temperature_global_mixed_hard_neg_all_8m8g_1point5e-5-two_strategy_train_temperature_held-out"                           # 负样本的来源
RERANK_MODEL_NAME = "qwen2-vl-7b_BiLamRA_Ret_cc3m_llm_8m8g_4xlr_train_temperature_held-out-Rank-Only-Pointwise"                                                 # rerank 模型名称                                     
MODEL_LOCAL_PATH = "./checkpoints/held-out/qwen2-vl-7b_BiLamRA_Ret_cc3m_llm_8m8g_4xlr_train_temperature_global_mixed_hard_neg_all_8m8g_1point5e-5-two_strategy_train_temperature_held-out-merged"       # 基座模型
QUERY_DATA_PATH = "./data/M-BEIR/query/union_train/mbeir_union_up_train_10percent_held-out.jsonl"
...
HARD_NEGATIVE_PATH = f"./result/score/held-out/{RERANK_MODEL_NAME}/10percent/{SOURCE_MODEL_NAME}/local/fusion_results.jsonl"
MODALITY_HARD_NEGATIVE_PATH = f"./result/score/held-out/{RERANK_MODEL_NAME}/10percent/{SOURCE_MODEL_NAME}/global/fusion_results.jsonl"
QUERY_FEATURE_PATH = f"./result/result_eval_train/held-out/10percent/{SOURCE_MODEL_NAME}/global/all_query_names2query_features.pth" #  query 特征约束路径
CAND_FEATURE_PATH = f"./result/result_eval_train/held-out/10percent/{SOURCE_MODEL_NAME}/global/all_cand_ids2cand_features.pth"      #  cand 特征约束路径
...
# 创建结果目录
os.makedirs(f"./checkpoints/held-out/{RUN_ID}", exist_ok=True)
# 创建日志文件位置
log_file_path = f"./checkpoints/held-out/{RUN_ID}/{TIMENAME}stdout.log"
# 重定向标准输出到日志文件
...
torchrun_command = [
    ...
    *DISTRIBUTED_ARGS.split(),
    "train/train_mbeir_with_distillrerank.py",
    "--model_id", MODEL_ID,
    ...
    "--output_dir", f"./checkpoints/held-out/{RUN_ID}",
