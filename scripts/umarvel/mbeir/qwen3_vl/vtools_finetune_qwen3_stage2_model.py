...
try:
    ...
    os.environ['NPROC_PER_NODE'] = '2' # 单机单卡 debug 训练时设置为1
    # ==== 分布式参数 ====
    MASTER_ADDR = os.environ.get('MASTER_ADDR')             # master_ip
    MASTER_PORT = 10060  # 主节点端口
    NNODES = int(os.environ.get('NNODES'))                  # workers 总节点数
...
RUN_ID = f"{MODEL_ID}_m-beir_stage2_model"                                                        # 模型名称
SOURCE_MODEL_NAME = "qwen3-vl-8b_m-beir_stage1_model"                                             # 负样本的来源
MODEL_LOCAL_PATH = "./checkpoints/qwen3-vl-8b_m-beir_stage1_model-merged"                         # 基座模型
# MODEL_LOCAL_PATH = "./checkpoints/hf_models/Qwen3-VL-8B-Instruct"                               # 基座模型
...
HARD_NEGATIVE_PATH = f"./result/result_eval_train/{SOURCE_MODEL_NAME}/local/rerank_data_all_eval_train_local.jsonl"
MODALITY_HARD_NEGATIVE_PATH = f"./result/result_eval_train/{SOURCE_MODEL_NAME}/global/rerank_data_all_eval_train_global.jsonl"
...
PER_DEVICE_BATCH_SIZE = 12                  # 实验的时候记得修改成 14，保持 50*2=100 的特征数量; 14跑的时候失败了改成12试试
GRAD_ACCUM = 1
NUM_EPOCHS = 1
LR = 1.5e-5
MODEL_MAX_LEN = 1024
...
torchrun_command = [
    ...
    *DISTRIBUTED_ARGS.split(),
    "train/train_mbeir_with_mixed_hard_neg_sample_qwen3_vl.py",
    "--model_id", MODEL_ID,
