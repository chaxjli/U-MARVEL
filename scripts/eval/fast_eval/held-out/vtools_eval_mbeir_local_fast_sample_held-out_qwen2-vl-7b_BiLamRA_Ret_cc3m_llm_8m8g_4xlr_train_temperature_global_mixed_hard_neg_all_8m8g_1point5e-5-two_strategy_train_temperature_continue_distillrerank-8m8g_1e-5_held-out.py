...
# 模型评估参数   -----  请根据实际情况修改
MODEL_NAME="qwen2-vl-7b_BiLamRA_Ret_cc3m_llm_8m8g_4xlr_train_temperature_global_mixed_hard_neg_all_8m8g_1point5e-5-two_strategy_train_temperature_continue_distillrerank-8m8g_1e-5_held-out"
MODEL_ID = f"./checkpoints/held-out/{MODEL_NAME}"
ORIGINAL_MODEL_ID ="./checkpoints/held-out/qwen2-vl-7b_BiLamRA_Ret_cc3m_llm_8m8g_4xlr_train_temperature_global_mixed_hard_neg_all_8m8g_1point5e-5-two_strategy_train_temperature_held-out-merged"
...
# 创建结果目录
os.makedirs(f"./result/result_fast_sample/held-out/{MODEL_NAME}_eval_results_finetune/local", exist_ok=True)
total_log_file = f"./result/result_fast_sample/held-out/{MODEL_NAME}_eval_results_finetune/local/{TIMENAME}total_stdout.log"
...
command = [
    ...
    "eval/eval_mbeir_local_finetune_fast_sample_held-out.py",
    "--instructions_path=./data/M-BEIR/instructions/query_instructions.tsv",
    f"--save_dir_name=./result/result_fast_sample/held-out/{MODEL_NAME}_eval_results_finetune/local",
