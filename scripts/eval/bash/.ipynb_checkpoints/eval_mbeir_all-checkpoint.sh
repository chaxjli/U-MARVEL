...
for i in "${!query_data_paths[@]}"; do
    ...
    # 构建要检查的文件路径,输出路径 ./result./LamRA_Ret_eval_results/
    check_file="./result./LamRA_Ret_eval_results/${result}_qwen2-vl-7b_LamRA-Ret_candidate_features.pth"
    ...
    if [ -f "$check_file" ]; then
        ...
        accelerate launch --multi_gpu --main_process_port "$CURRENT_PORT" eval/eval_mbeir.py \
...
# 评估 global 版本结果
python eval/eval_mbeir_global_debug.py        # 评估自己的模型
...
#     result=$(echo "$filename" | sed 's/_qrels.txt//')
#     # 构建要检查的文件路径
#     check_file="./LamRA_Ret_eval_results_author_model/${result}_LamRA-Ret_candidate_features.pth"
...
#         echo "文件 $check_file 存在，跳过本次任务。"
#     else
#         CUDA_VISIBLE_DEVICES='0,1,2,3,4,5,6,7' accelerate launch --multi_gpu --main_process_port "$CURRENT_PORT" eval/eval_mbeir_author_model.py \
#             --query_data_path "${query_data_paths[$i]}" \
#             --query_cand_pool_path "${query_cand_pool_paths[0]}" \
#             --cand_pool_path "${cand_pool_paths[$i]}" \
#             --instructions_path ./data/M-BEIR/instructions/query_instructions.tsv \
...
source /home/user_name/miniconda3/bin/activate && conda activate u-marvel_faiss
export LD_LIBRARY_PATH=/usr/local/Ascend/driver/lib64:/usr/local/Ascend/add-ons:$LD_LIBRARY_PATH
export PYTHONPATH=/usr/local/Ascend/pyACL/python/site-packages:$PYTHONPATH
export PATH="/home/user_name/miniconda3/envs/u-marvel/bin/:$PATH"

# python eval/eval_mbeir_global_author_model.py # 评估作者的模型
