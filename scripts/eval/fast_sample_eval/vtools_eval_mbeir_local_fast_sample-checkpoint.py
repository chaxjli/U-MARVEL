...
# 模型评估参数   -----  请根据实际情况修改
MODEL_NAME="xxxxxxxx"  # 模型名称
checkpoint_num = "600"
MODEL_ID = f"./checkpoints/{MODEL_NAME}/checkpoint-{checkpoint_num}"
ORIGINAL_MODEL_ID ="./checkpoints/xxxxxxxx"
...
# 创建结果目录
os.makedirs(f"./result/result_fast_sample/{MODEL_NAME}_eval_results_finetune/checkpoint-{checkpoint_num}/local", exist_ok=True)
total_log_file = f"./result/result_fast_sample/{MODEL_NAME}_eval_results_finetune/checkpoint-{checkpoint_num}/local/{TIMENAME}total_stdout.log"
...
command = [
    ...
    "eval/eval_mbeir_local_finetune_fast_sample.py",
    "--instructions_path=./data/M-BEIR/instructions/query_instructions.tsv",
    f"--save_dir_name=./result/result_fast_sample/{MODEL_NAME}_eval_results_finetune/checkpoint-{checkpoint_num}/local",
