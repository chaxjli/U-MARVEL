...
try:    # 先尝试多机加载环境变量
    ...
    os.environ['NPROC_PER_NODE'] = '8'                      # 记得修改
    MASTER_ADDR = os.environ.get('MASTER_ADDR')             # master_ip
    NNODES = int(os.environ.get('NNODES'))                  # workers 总节点数
    NODE_RANK = int(os.environ.get('NODE_RANK'))            # worker_id 当前机器节点 Rank (主节点为 0)
    NPROC_PER_NODE = int(os.environ.get('NPROC_PER_NODE'))  # gpu_num 每个节点的 NPU 数量
...
MODEL_NAME = "qwen2-vl-7b_BiLamRA_Ret_cc3m_llm_8m8g_4xlr-Rank-Only-Pointwise"                       # 模型名称
MODEL_ID = f"./checkpoints/rerank_model/{MODEL_NAME}"                                               # rank 模型路径
SOURCE_MODEL_NAME = "qwen2-vl-7b_BiLamRA_Ret_cc3m_llm_8m8g_4xlr_global_mixed_hard_neg_all_8m8g_1point5e-5" # 提供第一阶段检索结果的模型名称

os.makedirs(f"./result/result_rank/train/{MODEL_NAME}/{SOURCE_MODEL_NAME}/local", exist_ok=True)
total_log_file = f"./result/result_rank/train/{MODEL_NAME}/{SOURCE_MODEL_NAME}/local/{TIMENAME}total_stdout.log"
...
for i, TASK_NAME in enumerate(TASK_NAMES):
    ...
    check_file = f"./result/result_rank/train/{MODEL_NAME}/{SOURCE_MODEL_NAME}/local/{TASK_NAME}_random50_train_queryid2rerank_score.json"
    print("====================================================================")
    ...
    if os.path.isfile(check_file):
        ...
        command = [
            ...
            "eval/eval_train_data_mbeir_rerank_pointwise_random100.py",
            f"--query_data_path=./data/M-BEIR/query/train/query_train/mbeir_train_{TASK_NAME}_dedup_10percent.jsonl",
            ...
            f"--ret_query_data_path=./result/result_eval_train/{SOURCE_MODEL_NAME}/local/mbeir_train_{TASK_NAME}_{SOURCE_MODEL_NAME}_new_query_names_random100.json",
            f"--ret_cand_data_path=./result/result_eval_train/{SOURCE_MODEL_NAME}/local/mbeir_train_{TASK_NAME}_{SOURCE_MODEL_NAME}_new_cand_names_random100.json",
            f"--save_dir_name=./result/result_rank/train/{MODEL_NAME}/{SOURCE_MODEL_NAME}/local/",
            "--rank_num=50",
            f"--save_name={TASK_NAME}_random50"
        ]
        try:
            # 打开总的日志文件以追加模式写入
