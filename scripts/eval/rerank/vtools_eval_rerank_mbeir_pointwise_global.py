...
# 提供第一阶段检索结果的模型名称
SOURCE_MODEL_NAME = "qwen2-vl-7b_BiLamRA_Ret_cc3m_llm_8m8g_4xlr"                                    # 提供第一阶段检索结果的模型名称

os.makedirs(f"./result/result_rank/{MODEL_NAME}/{SOURCE_MODEL_NAME}/global", exist_ok=True)
total_log_file = f"./result/result_rank/{MODEL_NAME}/{SOURCE_MODEL_NAME}/global/{TIMENAME}total_stdout.log"
...
for i, TASK_NAME in enumerate(TASK_NAMES):
    ...
    check_file = f"./result/result_rank/{MODEL_NAME}/{SOURCE_MODEL_NAME}/global/{TASK_NAME}_top50_test_queryid2rerank_score.json"
    print("====================================================================")
    ...
    if os.path.isfile(check_file):
        ...
        command = [
            ...
            f"--ret_query_data_path=./result/{SOURCE_MODEL_NAME}_eval_results_finetune/global/{TASK_NAME}_query_names.json",
            f"--ret_cand_data_path=./result/{SOURCE_MODEL_NAME}_eval_results_finetune/global/{TASK_NAME}_cand_names.json",
            f"--save_dir_name=./result/result_rank/{MODEL_NAME}/{SOURCE_MODEL_NAME}/global/",
            "--rank_num=50",
