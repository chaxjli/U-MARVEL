...
model_name = "qwen2-vl-7b_BiLamRA_Ret_cc3m_llm_8m8g_4xlr_global_mixed_hard_neg_all_8m8g_1point5e-5"   # 这里需要修改: 需要第一阶段模型名称
...
rerank_path_prefix = f"./result/score/{rerank_model_name}/{model_name}/local"
...
for task_name in task_names:
    ...
    raw_scores = load_json(f"./result/result_eval_train/{model_name}/local/mbeir_train_{task_name}_{model_name}_new_scores_random100.json")
    rerank_scores = load_json(f"./result/result_rank/train/{rerank_model_name}/{model_name}/local/{task_name}_random50_train_queryid2rerank_score.json")
    query_names = load_json(f"./result/result_eval_train/{model_name}/local/mbeir_train_{task_name}_{model_name}_new_query_names_random100.json")
    cand_names = load_json(f"./result/result_eval_train/{model_name}/local/mbeir_train_{task_name}_{model_name}_new_cand_names_random100.json")
    ...
    with open(f"{rerank_path_prefix}/{task_name}_fusion_scores_random50.json", "w") as f:
        json.dump(fusion_scores, f,ensure_ascii=False)
    with open(f"{rerank_path_prefix}/{task_name}_fusion_query_names_random50.json", "w") as f:
        json.dump(fusion_query_names, f, ensure_ascii=False)    
    with open(f"{rerank_path_prefix}/{task_name}_fusion_cand_names_random50.json", "w") as f:
        json.dump(fusion_cand_names, f, ensure_ascii=False)
    print(f"融合后的结果已保存到 {rerank_path_prefix}/{task_name}_fusion_scores_random50.json")
    print(f"融合后的结果已保存到 {rerank_path_prefix}/{task_name}_fusion_query_names_random50.json")
    print(f"融合后的结果已保存到 {rerank_path_prefix}/{task_name}_fusion_cand_names_random50.json")
...
with open(data_sample_file_path, 'r', encoding='utf-8') as f:
    ...
    for index,line in enumerate(tqdm(f, total = line_count,desc=f"Loading: ")):
        ...
        qid = item['qid']
        assert qid in fusion_results, f"qid {qid} 不在 fusion_results_random50 中"
        fusion_results_list.append(copy.deepcopy(fusion_results[qid]))

# 将 fusion_results_list 保存成 jsonl 文件
with open(f"{rerank_path_prefix}/fusion_results_random50.jsonl", "w") as f:
    for item in fusion_results_list:
        f.write(json.dumps(item, ensure_ascii=False) + "\n")
print(f"融合后的结果已保存到 {rerank_path_prefix}/fusion_results_random50.jsonl")
