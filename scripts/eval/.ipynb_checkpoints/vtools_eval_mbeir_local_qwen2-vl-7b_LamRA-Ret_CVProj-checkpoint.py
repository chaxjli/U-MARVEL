...
# 动态扩展内存段：允许NPU在运行时按需扩展内存分配，避免因预分配不足导致内存不足（OOM）或频繁申请/释放内存的开销
# 报错 NPU 显存不够的时候可以使用
...
MODEL_NAME="qwen2-vl-7b_LamRA-Ret_CVProj"
...
for i in range(len(query_data_paths)):
    ...
    # 构建要检查的文件路径
    check_file = f"./result/{MODEL_NAME}_eval_results_local/{result}_{MODEL_NAME}_candidate_features.pth"
    ...
    if os.path.isfile(check_file):
        ...
        command = [
            ...
            f"--save_dir_name=./result/{MODEL_NAME}_eval_results_local",
