...
MODEL_NAME="qwen2-vl-7b_LamRA_Ret_task_type_token_cls"
...
for i in range(len(query_data_paths)):
    ...
    # 构建要检查的文件路径
    check_file = f"./result/{MODEL_NAME}_eval_results_finetune/local/{result}_{MODEL_NAME}_candidate_features.pth"
    ...
    if os.path.isfile(check_file):
        ...
        command = [
            ...
            f"--save_dir_name=./result/{MODEL_NAME}_eval_results_finetune/local",  # 不需要带 local 因为后续的 global 也会写入这个目录
            f"--original_model_id={ORIGINAL_MODEL_ID}",
            f"--model_id={MODEL_ID}",
            f"--model_id={MODEL_ID}",
            # ">",
            # f"./result/{MODEL_NAME}_eval_results_finetune/local/{TIMENAME}stdout.log", # 重定向标准输出（stdout）
            # "2>",
        ...
        try:
            ...
            with open(total_log_file, 'a') as log_file:
                ...
                # 执行命令并将标准输出重定向到日志文件，忽略标准错误输出
                subprocess.run(command, stdout=log_file, stderr=subprocess.DEVNULL, check=True)
            print(f"任务 {i} 执行完成，日志已记录到 {total_log_file}")
