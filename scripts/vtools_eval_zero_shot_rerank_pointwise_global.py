...
# 创建结果目录
os.makedirs(f"./result/result_zero_shot_rerank/{MODEL_NAME}/{SOURCE_MODEL_NAME}/global", exist_ok=True)
total_log_file = f"./result/result_zero_shot_rerank/{MODEL_NAME}/{SOURCE_MODEL_NAME}/global/{TIMENAME}total_stdout.log"
...
tasks = [
    "mscoco_task3",
    # "mscoco_task0","visualnews_task0","fashion200k_task0","webqa_task1","edis_task2","webqa_task2",
    # "visualnews_task3","fashion200k_task3","nights_task4","oven_task6","infoseek_task6","fashioniq_task7",
    # "cirr_task7","oven_task8","infoseek_task8",
    ]

for TASK_NAME in tasks:
    # 检查该任务文件是否存在，如果存在则跳过
    check_file = f"./result/{MODEL_NAME}/{SOURCE_MODEL_NAME}/global/{TASK_NAME}_top50.json"
    ...
    command = [
        "accelerate", "launch", "--multi_gpu", "--main_process_port", "29508",
        ...
        "--ret_query_data_path", f"./result/{SOURCE_MODEL_NAME}_eval_results_finetune/global/{TASK_NAME}_query_names.json",
        "--ret_cand_data_path", f"./result/{SOURCE_MODEL_NAME}_eval_results_finetune/global/{TASK_NAME}_cand_names.json",
        "--rank_num", "50",
        "--save_name", f"{TASK_NAME}_top50"
        "--save_dir_name", f"./result/result_zero_shot_rerank/{MODEL_NAME}/{SOURCE_MODEL_NAME}/global/",
    ]
