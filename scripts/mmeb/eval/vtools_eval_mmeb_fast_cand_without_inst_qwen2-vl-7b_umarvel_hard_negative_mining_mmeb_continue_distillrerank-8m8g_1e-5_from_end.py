...
os.environ["CUDA_VISIBLE_DEVICES"] = "7"  # 仅可见 NPU
...
# 模型评估参数   -----  请根据实际情况修改
MODEL_NAME="qwen2-vl-7b_umarvel_hard_negative_mining_mmeb_continue_distillrerank-8m8g_1e-5"
MODEL_ID = f"./checkpoints/mmeb/{MODEL_NAME}"
ORIGINAL_MODEL_ID ="./checkpoints/mmeb/qwen2-vl-7b_umarvel_hard_negative_mining_mmeb-merged"
...
# 创建结果目录
os.makedirs(f"./result/{MODEL_NAME}_eval_results_finetune/mmeb_cand_without_inst", exist_ok=True)
total_log_file = f"./result/{MODEL_NAME}_eval_results_finetune/mmeb_cand_without_inst/{TIMENAME}total_stdout.log"
...
command = [
    ...
    "eval/eval_mmeb_finetune_cand_without_inst_from_end.py",
