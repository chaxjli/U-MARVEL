...
os.makedirs(f"./result/result_rank/mmeb/train/{MODEL_NAME}/{SOURCE_MODEL_NAME}/local/20percent", exist_ok=True)
total_log_file = f"./result/result_rank/mmeb/train/{MODEL_NAME}/{SOURCE_MODEL_NAME}/local/20percent/{TIMENAME}total_stdout.log"
...
# 定义所有任务列表
TASK_NAMES = IND[:]  # 使用 IND 列表中的任务名称
# 循环执行所有任务-------------------评估 pointwise
for i, TASK_NAME in enumerate(TASK_NAMES):
    # 动态计算端口（起始端口29500）
    CURRENT_PORT = 29500 + i
    check_file = f"./result/result_rank/mmeb/train/{MODEL_NAME}/{SOURCE_MODEL_NAME}/local/20percent/{TASK_NAME}_top50_train_queryid2rerank_score.json"
    print("====================================================================")
    ...
    if os.path.isfile(check_file):
        ...
        command = [
            ...
            "eval/eval_train_data_mmeb_rerank_pointwise.py",
            f"--query_data_path=./data/MMEB-train/{TASK_NAME}/{TASK_NAME}_query_20percent.jsonl",
            "--cand_pool_path=./data/MMEB-train/mmeb_union_candidate.jsonl",
            "--instructions_path=./data/MMEB-train/data_instruction.json",
            f"--model_id={MODEL_ID}",
            f"--original_model_id={ORIGINAL_MODEL_ID}",
            f"--ret_query_data_path=./result/mmeb/train/{SOURCE_MODEL_NAME}_eval_results_finetune/local/20percent/{TASK_NAME}_{SOURCE_MODEL_NAME}_query_names.json",
            f"--ret_cand_data_path=./result/mmeb/train/{SOURCE_MODEL_NAME}_eval_results_finetune/local/20percent/{TASK_NAME}_{SOURCE_MODEL_NAME}_cand_names.json",
            f"--save_dir_name=./result/result_rank/mmeb/train/{MODEL_NAME}/{SOURCE_MODEL_NAME}/local/20percent",
            "--rank_num=50",
